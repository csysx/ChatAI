...
4. 本地运行
可以在Android Studio 直接运行
    打开 Android Studio，导入项目（选择chatAI根目录）；
    等待 Gradle 同步完成（首次同步需下载依赖）；
    连接 Android 真机 / 启动模拟器（API 24+）；
    点击「Run 'app'」按钮（绿色三角），选择设备后运行。
...
2. 核心流程
会话创建与消息发送流程：
    用户点击「新建会话」→ SessionViewModel 创建会话（生成 UUID）→ 跳转到聊天页 → 
    用户输入文本 → ChatViewModel 存储用户消息 → 拼接上下文 → 调用 AI 接口 → 
    接收 AI 回复 → 存储 AI 消息 → 更新 UI 状态 → 渲染消息气泡
